...
wgpu::Adapter Library::RequestAdapter(wgpu::Instance instance) {
  ...
  auto adapterCallback = [](wgpu::RequestAdapterStatus status,
                            wgpu::Adapter _adapter, wgpu::StringView,
                            void *userdata) {
    if (status == wgpu::RequestAdapterStatus::Success)
      *reinterpret_cast<wgpu::Adapter *>(userdata) = std::move(_adapter);
  };

  constexpr wgpu::FeatureLevel kLevels[2] = {wgpu::FeatureLevel::Core,
  ...
  for (wgpu::FeatureLevel level : kLevels) {
    ...
    instance.WaitAny(
        instance.RequestAdapter(&adapterOptions, callbackMode, adapterCallback, reinterpret_cast<void*>(&adapter)),
        UINT64_MAX);

    if (adapter) {
      LOG_WARN("Selected adapter with {} feature level",
...
wgpu::Device Library::RequestDevice(wgpu::Adapter adapter) {
  ...
  auto deviceLostCallback =
      [](wgpu::Device const &, wgpu::DeviceLostReason reason,
         wgpu::StringView message) { LOG_ERROR("{}, {}", reason, message); };
  ...
  auto deviceCallback = [](wgpu::RequestDeviceStatus status,
                           wgpu::Device _device, const char *message,
                           void *userdata) {
  ...
  adapter.GetInstance().WaitAny(
      adapter.RequestDevice(&deviceDescriptor, deviceCallbackMode,
                            deviceCallback, deviceUserdata),
      UINT64_MAX);
