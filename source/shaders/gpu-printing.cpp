...
void GPUPrinting::loadStrings(slang::ProgramLayout* slangReflection)
{
    ...
    for (SlangUInt ii = 0; ii < hashedStringCount; ++ii) {
        ...
        char const* stringData =
            slangReflection->getHashedString(ii, &stringSize);
        ...
        //
        m_hashedStrings.insert(std::make_pair(
            hash, std::string(stringData, stringData + stringSize)));
    }
}
...
void GPUPrinting::processGPUPrintCommands(const void* data, size_t dataSize)
{
    ...
    if (dataWordCount < 1) {
        fprintf(stderr,
                "error: expected at least 4 bytes in GPU printing buffer\n");
        return;
    }
    //
    ...
    while (dataCursor < dataEnd) {
        ...
        switch (op) {
            default:
                // If we encounter an op that we don't understand, there is a
            ...
            case GPUPrintingOp::Nop:
                // The `Nop` case is a no-op, and allows GPU code to
                // conservatively allocate bytes in the printing buffer and then
                // overwrite any excess with zeros to trim their allocation.
                break;
...
#undef CASE
            ...
            case GPUPrintingOp::PrintF: {
                ...
                // provided by standard library on the host CPU, because we
                // don't have a portable way to translate the payload data into
                ...
                while (cursor != end) {
                    ...
                    if (cursor == end) {
                        fprintf(
                            stderr,
                            "error: unexpected '%%' at and of format string\n");
                        break;
                    }
                    ...
                    switch (specifier) {
                        default:
                            fprintf(stderr,
                                    "error: unexpected format specifier '%c' "
                                    "(0x%X)\n",
                                    specifier,
                                    specifier);
                            break;
